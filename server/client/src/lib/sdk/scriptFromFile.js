...
function scriptFromFile(filename, includeRempl = false) {
    const remplPath = path.join(
        path.dirname(url.fileURLToPath((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('sdk/scriptFromFile.js', document.baseURI).href)))),
        '../../dist/rempl.js'
    );
    let cache = null;
